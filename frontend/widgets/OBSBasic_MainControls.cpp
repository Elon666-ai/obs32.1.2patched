...
void OBSBasic::on_action_Settings_triggered()
{
	...
	// If manual ROI ended up enabled, land back on the preview with the
	// "Set ROI" tool already armed instead of making the user go click it
	// themselves - UpdateRoiSelectButton() first so the button is
	// shown/enabled before SetRoiSelectMode(true) checks it.
	UpdateRoiSelectButton();
	if (!outputHandler->StreamingActive() && config_get_bool(Config(), "Video", "RoiEnabled"))
